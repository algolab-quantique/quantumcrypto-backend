...
from dps.models import DPSGame, DPSPlayer, Game, DPSRoom, DPSIteration
import random
...
class WaitingRoomConsumer(AsyncJsonWebsocketConsumer):
    ...
    def get_game(self, game_code: str):
        return DPSGame.objects.get(code=game_code)

    @database_sync_to_async
    def delete_game(self, game: DPSGame):
        return game.delete()

    @database_sync_to_async
    def get_player(self, game: DPSGame, player_name: str):
        return DPSPlayer.objects.get(game_id=game, name=player_name)

    @database_sync_to_async
    def get_players(self, game: DPSGame):
        players = list(DPSPlayer.objects.filter(game_id=game).values())
        return players

    @database_sync_to_async
    def delete_player(self, player: DPSPlayer):
        return player.delete()

    @database_sync_to_async
    def create_player(self, player_name: str, game: DPSGame):
        try:
            player = DPSPlayer.objects.create(name=player_name, game_id=game)
    ...
    @database_sync_to_async
    def create_room(self, game, player1: DPSPlayer, player2: DPSPlayer,
                    eve_present: bool):
        try:
            room = DPSRoom.objects.create(game_id=game, player1=player1,
                                           player2=player2)
            iteration = DPSIteration.objects.create(room=room,
                                                     eve_present=eve_present)
            room.save()
            iteration.save()
        except Exception as e:
    ...
    @database_sync_to_async
    def save_game(self, game: DPSGame):
        try:
            game.save()
        except Exception as e:
    ...
    @database_sync_to_async
    def save_player(self, player: DPSPlayer):
        try:
            player.save()
        except Exception as e:
...
class PlayingRoomConsumer(AsyncJsonWebsocketConsumer):
    ...
    def update_iterations_status_to_finished(self, player_name, game_code):
        ...
        player = DPSPlayer.objects.get(name=player_name, game_id=game)

        # Get the room
        room = DPSRoom.objects.get(models.Q(player1=player) | models.Q(
            player2=player),
                                    game_id=game, )

        # Get the created iterations for the room
        created_iterations = DPSIteration.objects.filter(room=room,
                                                          status='CREATED')
...
class ResultsPageConsumer(AsyncJsonWebsocketConsumer):
    ...
    def get_rooms_with_iterations(self, game):
        rooms_queryset = DPSRoom.objects.filter(
